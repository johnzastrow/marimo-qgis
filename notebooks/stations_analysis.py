...
def _(mo):
    mo.md("""
    **What this demonstrates.** Loading CWOP weather stations, computing a
    geodesic distance matrix with `QgsDistanceArea`, and finding
    nearest/farthest pairs with Pandas.

    **Dependencies.** PyQGIS (its own headless `QgsApplication`) and `pandas`;
    the data file `notebooks/stations.gpkg` (must sit next to this notebook).
    No plugin/bridge required.

    **How it works.** Loads `stations.gpkg`, measures ellipsoidal distances
    with `QgsDistanceArea`, analyses them in Pandas. **▶ Run the cells in
...
def _():
    ...
    qgs.initQgis()
    return Qgis, QgsDistanceArea, QgsPointXY, QgsProject, QgsVectorLayer
